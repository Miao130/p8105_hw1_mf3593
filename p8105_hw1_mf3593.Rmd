...
when apply as.numeric, the vectors are converted to numeric vectors. The logical vector can be converted based on 1 corresponds to TRUE and 0 corresponds to FALSE. The factor vector can be converted to numeric factor where numeric codes corresponding to the factor levels are returned. Character vector is converted to numeric vector of the same values. This explains why we couldn't get mean of factor and character vectors before. We need to convert them.
